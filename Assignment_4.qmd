...
**Skriv ut i fra denne tabellen som argument for hvilke land vi tar med oss videre!! "kommenter begrensninger; 0 verdier, eu kandidat og når land ble eu kandidat.**
...
### 2. GDP per capita calculation (metode 1)
...
Skriv i dette avsnittet at orginalmodellen passer dårlig, og at logaritmic blir "hovedmodellen" vår.
vis orginal mot logaritmic i en facet i appendix

## Logaritmic - Flett denne sammen med orginalmodellen
...
*By exploring different functional forms, we are able to determine the best functional form for our estimates by looking at descriptive statistics to justify our selections. We found no significant difference by including a cubic function, and thus, concluded to utilize a logarithmic function as well as a quadratic model.*
...
huxreg(DE = partaDE, IT = partaIT, statistics = c("nobs", "r.squared","adj.r.squared", "statistic", "p.value"), 
...
huxreg("Model 1 DE" = lmlogDE, "Model 1 IT" = lmlogIT, statistics = c("nobs","r.squared","adj.r.squared", "statistic", "p.value"), 
       stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
huxreg("Model 1 DE" = lmquadDE, "Model 1 IT" = lmquadIT, statistics = c("Number of obervations" = "nobs","r.squared","adj.r.squared", "statistic", "p.value"), 
       stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
#| echo: FALSE
#| tbl-cap: The effect of log squared regional development on inequality
lmqubicDE <- lm(gini_n2 ~ logGDPpcn2cubic, subset(merged_data, nuts0 == "DE" & gini_n2 > 0))

lmqubicIT <- lm(gini_n2 ~ logGDPpcn2cubic, subset(merged_data, nuts0 == "IT" & gini_n2 > 0))

#Tar denne vekk fordi vi ikke skal legge til sverige før i panel#lmquadSE <- lm(gini_n2 ~ logGDPpcn2quad, subset(merged_data, nuts0 == "SE" & gini_n2 > 0))

huxreg("Model 1 DE" = lmqubicDE, "Model 1 IT" = lmqubicIT, statistics = c("Number of obervations" = "nobs","r.squared","adj.r.squared", "statistic", "p.value"), 
       stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
#HUSK Å LEGG TIL FLERE LAND!! leg også til qubic?
```
