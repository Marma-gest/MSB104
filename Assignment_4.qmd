---
title: "The effect of regional development on regional inequality"
format: html
editor: visual
...
Understanding the reasoning for regional inequality is a complex topic, and understanding how economic growth, educational disparities, and spatial factors contribute to regional inequalities is crucial for informed policymaking.
In the context of this research, our focus is on a select group of countries, each with its unique socio-economic landscape, to delve into the nuanced dynamics of regional development and inequality. 

Laying the foundation for our analysis involves selecting a dataset encompassing several designated countries.
This initial step is vital in identifying the key nations that will be subject to closer scrutiny in subsequent assignments.
Furthermore, we extend our investigation to the year 2010, employing cross-sectional estimation techniques.
This step allows us to observe the state of regional development and inequality at a specific point in time.
Additionally, we introduce variables such as education, country area, and amount of highway infrastructure measured in kilometers to enrich our analysis and offer a more comprehensive understanding of the factors influencing regional dynamics.
Finally, we test the effectiveness of logarithmic, quadratic, and cubic models, considering their implications for understanding the relationship between regional development and inequality.
Fixed effects are introduced to account for nuanced variations across countries, providing a more nuanced and robust perspective.

This research holds significance in shedding light on the multifaceted relationship between regional development and inequality.
By employing diverse methodologies and incorporating essential variables, we aim to contribute nuanced insights that can inform policymakers and stakeholders in devising effective strategies for promoting balanced regional growth and mitigating inequality.
The importance of understanding these dynamics lies in its potential to foster inclusive development and ensure that the benefits of economic progress are equitably distributed across regions.
...
The first step is to estimate a regression model for the year 2010, with regional development, measured in GDP per capita, as the independent variable and regional inequality, measured in Gini, as the dependent variable.
...
GDP measure a country as a whole, where the GDP per capita is measuring the productivity of the population.
...
1.  Linearity: There must be a linear relationship between the dependent variable (regional GINI) and independent variable (GDP per capita).
    We can read from the two scatterplots, @fig-scatterplot-RL-DE and @fig-scatterplot-RL-IT, that the relationship for both Germany and Italy seems to be non-linear, with only some few outliers.
...
lmDE <- lm(gini_n2 ~ log(GDPpcn2) + education + log(area) + Roadkm, subset(merged_data, year == 2010 & nuts0 == "DE" & gini_n2 > 0))
lmDE1 <- lm(gini_n2 ~ logGDPpcn2quad + education + log(area) + Roadkm, subset(merged_data, year == 2010 & nuts0 == "DE" & gini_n2 > 0))
lmDE2 <- lm(gini_n2 ~ logGDPpcn2cubic + education + log(area) + Roadkm, subset(merged_data, year == 2010 & nuts0 == "DE" & gini_n2 > 0))

lmIT <- lm(gini_n2 ~ log(GDPpcn2) + education + log(area) + Roadkm, subset(merged_data, year == 2010 & nuts0 == "IT" & gini_n2 > 0))
lmIT1 <- lm(gini_n2 ~ logGDPpcn2quad + education + log(area) + Roadkm, subset(merged_data, year == 2010 & nuts0 == "IT" & gini_n2 > 0))
lmIT2 <- lm(gini_n2 ~ logGDPpcn2cubic + education + log(area) + Roadkm, subset(merged_data, year == 2010 & nuts0 == "IT" & gini_n2 > 0))
...
The table presents the results of regression models for Germany (DE) and Italy (IT) using logarithmic, quadratic, and cubic functional forms to explore the relationship between GDP per capita (log(GDPpcn2)) and regional inequality.
In the logarithmic model, the intercept is significantly negative for both countries, suggesting a baseline negative impact on inequality.
The coefficient for log(GDPpcn2) is positive and highly significant for both countries, indicating that an increase in GDP per capita is associated with higher inequality.

Moving to the quadratic model, the coefficients for the quadratic terms (logGDPpcn2quad) are positive and statistically significant for both countries.
This suggests a convex relationship between GDP per capita and inequality, indicating diminishing returns to economic development in terms of reducing inequality.

In the cubic model, the coefficients for the cubic terms (logGDPpcn2cubic) are positive and highly significant for both countries, indicating a more pronounced curvature in the relationship between GDP per capita and inequality.

The R-squared values show a modest improvement from the logarithmic to the cubic model, suggesting that the inclusion of higher-order terms contributes to explaining more variability in inequality.
However, the adjusted R-squared values are relatively lower, indicating potential overfitting.

Overall, the models provide insights into the non-linear relationship between GDP per capita and regional inequality, with evidence of diminishing returns and a more complex curvature as additional higher-order terms are included.
The statistical significance of these terms suggests their relevance in capturing the underlying dynamics of this relationship.
...
In the presented table, we provide an overview of the alternative functional forms applied to the time span from 2000 to 2020 for both Germany and Italy.
The models encompass logarithmic (DElog, ITlog), quadratic (DEquad, ITquad), and cubic (DEcubic, ITcubic) specifications.
It's noteworthy that the cubic model exhibits a marginally better goodness of fit compared to the quadratic model.
However, in alignment with our theoretical framework, we opt to utilize the quadratic specification, as it better aligns with our theoretical expectations, whereas the cubic model does not.
This decision ensures the coherence between the chosen functional form and the underlying conceptual framework of our analysis.

The two figures below, @fig-logGDPpcn2DE and @fig-logGDPpcn2IT visualize a quadratic relationship between log(GDP) and inequality for Germany and Italy.
...
We will now proceed to summarize fixed effects in a table below.
This analysis focuses on the countries Austria, Belgium, Bulgaria, Germany, Italy, and Sweden, considering fixed effects at NUTS 0, NUTS 2, the year level, and the combination of NUTS 2 and year.
Fixed effects play a crucial role in our study as they allow us to incorporate all these countries into our analysis, even those with fewer observations.
By applying fixed effects, we aim to mitigate the impact of factors that could influence our results, enabling a comprehensive examination of the selected European countries.
...
In the FE Nuts0 model, the coefficient for log(GDPpcn2) is significantly negative (-8.733, p \< 0.01), suggesting a substantial impact on regional inequality.
However, the FE Year model shows a less pronounced negative effect (-4.098, p \< 0.05).
Moving to FE Nuts2, the coefficient for log(GDPpcn2) turns positive (0.741), indicating a shift in the relationship, but it lacks statistical significance.
The FE Nuts2 and Year model presents a significant positive effect for log(GDPpcn2) (1.931, p \< 0.05), implying a potential interaction effect over time and across regions.
The quadratic and cubic terms demonstrate nuanced effects, with quadratic terms generally significant and cubic terms showing less consistent significance.
The adjusted R-squared values vary across models, underscoring the complexity of the relationship between GDP and inequality under different fixed effects specifications.

# V. Discussion

## Key Insights

The empirical analysis of regional development and inequality in Germany and Italy yields key insights into the complex relationship between GDP per capita (log(GDPpcn2)) and regional inequality.

The non-linear nature of the relationship emerges as a central theme, evidenced by the quadratic and cubic functional forms.
This complexity suggests diminishing returns of regional development in terms of reducing inequality.
Comparing alternative functional forms reveals the importance of the quadratic specification.
Despite a slightly better goodness of fit observed in the cubic model, the decision to opt for the quadratic model aligns with the theoretical framework, providing robustness to the analysis.

Fixed effects analysis adds layers of complexity to the findings.
Varying coefficients under different fixed effects structures highlight the importance of considering regional and temporal factors in understanding the impact of regional development on inequality.
Visualizations further enrich the narrative.
Figures 1 to 4 vividly illustrate the estimated relationships, depicting non-linear patterns in the impact of GDP per capita on inequality.
Germany and Italy exhibit distinct trends, emphasizing the need for region-specific policy considerations.

## Policy Considerations

Building upon the key insights, the analysis offers critical policy considerations for crafting effective and nuanced interventions in the realms of regional development and inequality.

Recognizing the non-linear relationship, policymakers should craft interventions that suit the developmental stage of each region.
Strategies promoting economic growth need to be nuanced, considering the varying effects on inequality.
Policymakers should account for regional and temporal factors when designing policies.
The shifting relationships observed under different fixed effects structures underscore the need for adaptive and context-specific interventions.

Policymakers must continuously track regional inequality trends to ensure the relevance and effectiveness of policies over time.
Timely adjustments can enhance the impact of interventions and address emerging challenges.
Recognizing diminishing returns in the quadratic relationship, policymakers should strategically allocate resources.
Focused investments in areas where the impact is most significant can lead to more efficient outcomes.
Policymakers should approach models with higher-order terms cautiously.
The non-linear relationships suggest that the impact of economic development on inequality is context-dependent, necessitating thoughtful policy responses.

In summary, these policy considerations provide a robust foundation for informed policy decisions, enabling policymakers to navigate the complexities and implement targeted strategies that promote inclusive and sustainable regional growth.

# VI. Limitations and Future Research

In our study of regional development and inequality, mainly in Germany and Italy, certain limitations suggest consideration.
The applicability of our findings beyond these specific regions may be constrained, given potential localized factors.
Our temporal scope, covering 2000 to 2020, might overlook evolving conditions.
Future research could extend the analysis to diverse regions and a more extended timeframe.
Micro-level investigations into education, infrastructure, and policies could enhance understanding.
Dynamic modeling, policy impact assessments, and additional variables would contribute to a more comprehensive analysis.
Robustness checks and alternative methodologies can further validate our findings.
Acknowledging these limitations, future research can provide a more nuanced exploration of regional development and inequality dynamics.

# VII. Conclusion

In conclusion, our comprehensive exploration of regional development and inequality dynamics in Germany and Italy yielded crucial insights.
No matter how we transform GDP, its impact on trying to explain regional inequality is minimal.
This implies that GDP per capita alone is not enough to explain regional inequality and that other omitted variables are likely better at explaining the variance.
As seen in the cross-selection estimations, increasing the number of explanatory variables such as population and geographical data, helped increase the explanatory power of regional inequality. 

The unexpected results underscore the complexity of regional dynamics.
While our study adds a layer of nuance to the discourse, further research is warranted to unravel the intricate factors influencing regional inequality.
This reflection emphasizes the dynamic nature of regional development and the continuous need for nuanced investigations to inform effective policy strategies.
