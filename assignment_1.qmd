...
educ <- educ |> 
  ...
  mutate(
    ...
    mutate(
    sumeduc2 = sum(education, na.rm = TRUE) / 100
  ) |> 
    slice(1) |> 
    ungroup() |> 
  select(-age)
...
lmeduc <- lm(gini_n2 ~ log(GDPpcn2) + education + log(area), subset(merged2010, year == 2010 & nuts0 == "DE" & gini_n2 > 0))

lmeduc2 <- lm(gini_n2 ~ log(GDPpcn2) + education + log(area), subset(merged2010, year == 2010 & nuts0 == "IT" & gini_n2 > 0))
...
huxreg("Model 1 DE" = lm1, "Model 1 IT" = lm2, "Model 2 DE" = lmeduc, "Model 2 IT" = lmeduc2, "Model 3 DE" = lm3, "Model 3 IT" = lm4, statistics = c("r.squared","adj.r.squared", "statistic", "p.value"))
```
