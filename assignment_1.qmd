...
roads <- roads |> 
  select(
    geo, Roadkm = OBS_VALUE, year = TIME_PERIOD
  ) 
...
```{r}
lmDETest <- lm(log(gini_n2) ~ log(education), subset(merged_data, year == 2010 & nuts0 == "DE" & gini_n2 > 0))
lmITTest <- lm(log(gini_n2) ~ log(education), subset(merged_data, year == 2010 & nuts0 == "IT" & gini_n2 > 0))
