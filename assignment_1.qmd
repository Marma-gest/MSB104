...
In order to achieve a presentable figure, our analysis focuses on the NUTS2 level for Germany rather than NUTS3 due to the larger scale and has been split into North- and South Germany.
This allows for a more comprehensive examination of regional variations, and captures a broader perspective on regional dynamics and economic disparities.

The zero values present in the data occur as a result of regions that are actually defined as a region, but in reality are a foreign embassy, army base or offshore energy production with no permanent residence.
This means they do not have any GDP per capita [@eurostat2021].
Moreover, we assume that the zero values can also be a result of missing data report from different regions in Germany.
...
merged_data |>
  filter(str_detect(geo, "DE30|DE40|DE50|DE60|DE80|DE91|DE92|DE93|DE94|DEA1| DEA2|DEA3|DEA4|DEA5|DEE0|DEF0|DEG0")) |> 
  ggplot(mapping = aes(
  x = year, 
  y = GDPpcn2, 
  colour = nuts2
...
# UNFINISHED SOUTH DE, DEB, DEC
merged_data |>
  filter(str_detect(nuts2, "DE11|DE12|DE13|DE14|DE21|DE22|DE23|DE24|DE25|DE26|DE27|DE71|DE72|DE73|DEB1|DEB2|DEB3|DEC0|DED2|DED4|DED5")) |> 
  ggplot(mapping = aes(
    x = year, 
    y = GDPpcn2, 
    colour = nuts2
...
huxreg(DE = lmDE, IT = lmIT, SE = lmSE, statistics = c("r.squared","adj.r.squared", "statistic", "p.value"), stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
...
huxreg("Model 1 DE" = lm1, "Model 1 IT" = lm2, "Model 2 DE" = lmarea, "Model 2 IT" = lmarea2, "Model 3 DE" = lm3, "Model 3 IT" = lm4, statistics = c("r.squared","adj.r.squared", "statistic", "p.value"), stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
In Assignment 3, which involves explaining alternative functional forms and implementing panel estimates to analyze the relationship between regional development and inequality, we will focus on the countries Sweden, Germany, and Italy.
These three countries have been selected due to their status as the three largest in our dataset, containing the largest amount of available data.
...
huxreg(DE = partaDE, IT = partaIT, SE = partaSE, statistics = c("Number of observations" = "nobs", "r.squared","adj.r.squared", "statistic", "p.value"), 
...
In Italy (IT), the intercept is -0.011 and it is not statistically significant.
The GDPpcn2 coefficient, at 0.000 (p \< 0.01), highlights a negligible association with inequality.
The GDPpcn2 coefficient for Sweden is negative (-0.009) which suggest that an increase in GDP per capita reduces inequality.
The coefficient, at 0.000 (p \< 0.01), mirrors Germany's minimal effect on inequality.
...
We found no significant difference by including a cubic function, and thus, concluded to utilize a logarithmic function as well as a quadratic model.
...
huxreg("Model 1 DE" = lmlogDE, "Model 1 IT" = lmlogIT, "Model 1 SE" = lmlogSE, statistics = c("Number of observations" = "nobs","r.squared","adj.r.squared", "statistic", "p.value"), 
       stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
huxreg("Model 1 DE" = lmquadDE, "Model 1 IT" = lmquadIT, "Model 1 SE" = lmquadSE, statistics = c("Number of observations" = "nobs","r.squared","adj.r.squared", "statistic", "p.value"), 
       stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
#| cap-location: bottom
#| fig-cap: "Scatter Plot for Sweden with a regression line showing the relationship between the gini coefficient and GDP per capita."
#| 
merged_data |> 
...
Sweden has a similar relationship between regional inequality and regional development as Italy, but at a slightly lower level, suggesting that regional inequality is lower in Sweden than in Italy, without regards to the regional development.
...
As discussed earlier, the original linear model indicates a significant but marginal impact of GDP per capita (GDPpcn2) on inequality across all three countries.
...
In the logarithmic alternative model the relationship between regional development and inequality is captured in a log-linear fashion.
The coefficients for Germany, Italy, and Sweden reveal significant associations.
Notably, the logarithmic model does not improve the fit, as indicated by the decreased adjusted R-squared values.

The quadratic alternative model introduces a quadratic term to accommodate potential curvature in the relationship.
...
Only the alternative quadratic model offer improvements over the original linear model.
The quadratic model introduces a curvature component, suggesting a more complex interplay between regional development and inequality.
The alternative model present enhanced explanatory power, capturing potential nonlinearities in the intricate relationship between regional development and inequality across Germany, Italy, and Sweden.

## Part C: Heteroskedasticity testing and causality discussion

### 1. Heteroskedasticity testing

In the original model, the extremely low p-value suggests evidence of heteroskedasticity for both Germany (DE) and Italy (IT).
For Sweden (SE), the high p-value suggests no strong evidence of heteroskedasticity.

The same picture can be observed for the alternative models (logarithmic and quadratic), were both Germany (DE) and Italy (IT) have extremely low p-values suggesting evidence of heteroskedasticity, while Sweden (SE) has a high p-value suggesting no strong evidence of heteroskedasticity.

**!!! We have observed heteroskedasticity in our data (see appendix x). .....**

### 2. Causality discussion

The presence of heteroskedasticity implies that the variance of the error terms is not constant across observations.
For models exhibiting heteroskedasticity (DE, IT), the standard errors of coefficient estimates may be biased, affecting the reliability of hypothesis tests.
The models for Sweden (SE) do not show strong evidence of heteroskedasticity.

To address heteroskedasticity, one strategy is to use robust standard errors or transform variables to achieve homoskedasticity.
Additionally, exploring other functional forms or including relevant predictors may help address the issue.
Reflecting on the causal interpretation, it's essential to acknowledge that heteroskedasticity doesn't necessarily invalidate causal relationships but emphasizes the importance of addressing statistical assumptions for more reliable inference.

1.  Homoskedasticity: To fulfill the OLS assumptions, variance in the observations needs to be equal, known as homoskedasticity. There are many tests for heteroskedasticity. In each case, the null hypothesis is homoskedasticity and the alternative is heteroskedasticity. If the variance in the observations are not equal, heteroskedasticity could be present. This may result in incorrect standard errors that are computed for the least squares estimators, and could lead to incorrect conclusions. In order to check for heteroskedasticity, we utilize the Breusch-Pagan test, **see appendix model X**
...
plm4 <- plm(gini_n2 ~ GDPpcn2 + log(GDPpcn2), data = pmerged, index = c( "year","nuts2"), model = "within",effect = "twoway") %>%
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)
huxreg(plm1, plm2, plm3, plm4, statistics = c("Number of observations" = "nobs","r.squared","adj.r.squared", "statistic", "p.value"), 
       stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
The coefficients for GDPpcn2 and log(GDPpcn2) are statistically significant in Models (1), (3), and (4), indicating a potential association with the dependent variable.
However, the lack of significance in Model (2) for both variables raises questions about their contribution in that specification.
Examining effect sizes, the negative and substantial magnitudes of log(GDPpcn2) in Models (1), (3), and (4) suggest a meaningful impact on the dependent variable.
The negative coefficient for GDPpcn2 in Model (1) is also noteworthy.
Adjusted R-squared values are negative in Models (1) and (2), which is unusual and suggests potential overfitting or model misspecification.
The statistical tests (statistic) indicate overall model significance.
Model (4) has the highest statistic, suggesting that the predictors collectively have a significant effect on the dependent variable.
...
Models 2 and 3 exhibit a significant effect, indicating the influence of time and regional fixed effects on the relationship.
Model 1 reveals a significant negative effect, suggesting that increased regional development reduces regional inequality (p \< 0.001).
Model 4 demonstrates a significant positive effect, indicating that, considering both country and time fixed effects, increased regional development increases regional inequality (p \< 0.001).

log(GDPpcn2):

In Model 1, a significant positive effect suggests a positive association between the logarithm of GDP per capita and regional inequality (p \< 0.001).
Model 2 reveals a significant negative effect, suggesting a negative association when considering only time fixed effects (p \< 0.05).
Model 3 indicates a significant negative effect, suggesting a negative association when considering only regional fixed effects (p \< 0.01).
Model 4 demonstrates a significant negative effect, indicating a negative association when considering both country and time fixed effects (p \< 0.001).

Comparison of Fixed Effects Models:

Model 1 contains Country Fixed Effects.
The positive GDPpcn2 coefficient suggests increased regional development reduces regional inequality.
The negative log(GDPpcn2) coefficient suggests an increase in GDP per capita positively correlates with regional inequality.

Models 2 and 3 contain Time and Regional Fixed Effects.
There is no significant effect of GDPpcn2, while the negative log(GDPpcn2) coefficients suggest a negative association with regional inequality.

Model 4 contains both Country and Time Fixed Effects.
The positive GDPpcn2 coefficient suggests increased regional development increases regional inequality, while the negative log(GDPpcn2) coefficient suggests a negative association with regional inequality.

By using panel data estimates, constants that might affect our estimates are removed, leveraging time data as these effects become obsolete.
This provides a better estimate on our variables, accounting for specific time-constant factors and fixed effects.
...
bp_pvalues <- c(
  ...
  bpSE = bptest(partaSE)$p.value,
  #logarithmic models
  bplogDE = bptest(lmlogDE)$p.value,
  bplogIT = bptest(lmlogIT)$p.value,
  bplogSE = bptest(lmlogSE)$p.value,
  #Quadratic models
  bpquadDE = bptest(lmquadDE)$p.value,
  bpquadIT = bptest(lmquadIT)$p.value,
  bpquadSE = bptest(lmquadSE)$p.value
) 
```

*A.1 Test results for heteroskedasticity using a Breusch-Pagan test*
