...
# UNFINISHED SOUTH
merged_data |>
 filter(str_detect(geo, "DE11|DE12|DE13|DE14|DE21|DE22|DE23|DE24|DE25|DE26|DE27|DE71|DE72|DE73|DEB1|DEB2|DEB3|DEC0|DED2|DED4|DED5")) |> 
  ggplot(mapping = aes(
  x = year, 
  y = GDPpcn2, 
  colour = nuts2
...
5.  Homoskedasticity: To fulfill the OLS assumptions, variance in the observations needs to be equal, known as homoskedasticity.
    ...
    In order to check for heteroskedasticity, we utilize the Breusch-Pagan test.
...
# The test statistic is 4.0861 and the corresponding p-value is x Since the p-value is not less than 0.05, we fail to reject the null hypothesis. We do not have sufficient evidence to say that heteroscedasticity is present in the regression model.

bptest(partaDE)$p.value
bptest(partaIT)$p.value
bptest(partaSE)$p.value
```
...
In Figure 1, we plot X against Y and illustrate the estimated relationship with a blue line

As seen above, the inequality (y-axis) starts off with a slight decrease as the GDP per capita (x-axis) increases.
However, inequality seem to increase.
Suggesting that a slight increase in GDP per capita will reduce inequality, but increase as it regional development increases.
Looking at the graph, we can see that a 0.2% increase from 10.2 to 10.4 increases the inequality about
...
huxreg(DE = partaDE, IT = partaIT, SE = partaSE, statistics = c("nobs", "r.squared","adj.r.squared", "statistic", "p.value"), 
...
Using a logarithmic model creates a asymptotic behaviour for the regression line.
...
huxreg("Model 1 DE" = lmlogDE, "Model 1 IT" = lmlogIT, "Model 1 SE" = lmlogSE, statistics = c("nobs","r.squared","adj.r.squared", "statistic", "p.value"), 
       stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
huxreg("Model 1 DE" = lmquadDE, "Model 1 IT" = lmquadIT, "Model 1 SE" = lmquadSE, statistics = c("nobs","r.squared","adj.r.squared", "statistic", "p.value"), 
       stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
       note = "Note): {stars} T statistics in brackets.")
```
...
bptest(partaSE)$p.value
#logarithmic models
bptest(lmlogDE)$p.value
bptest(lmlogIT)$p.value
bptest(lmlogSE)$p.value
#Quadratic models
