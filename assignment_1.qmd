...
```{r}
#Vi bruker ikke denne grunnet pmerged funker til dette?
...
```{r}
plm1 <- plm(gini_n2 ~ GDPpcn2 + log(GDPpcn2), data = pmerged, index = c("nuts0"), model = "within") %>%
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)


plm2 <- plm(gini_n2 ~ GDPpcn2 + log(GDPpcn2), data = pmerged, index = c("year"), model = "within") %>%
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)


plm3 <- plm(gini_n2 ~ GDPpcn2 + log(GDPpcn2), data = pmerged, index = c("nuts2"), model = "within") %>%
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)


plm4 <- plm(gini_n2 ~ GDPpcn2 + log(GDPpcn2), data = pmerged, index = c("nuts2", "year", "nuts0"), model = "within") %>%
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)
...
# ROT
plm(gini_n2 ~ GDPpcn2 + log(GDPpcn2), data = pmerged , index = c("nuts2", "year"), model = "within") %>%
    coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)
