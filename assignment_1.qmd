...
1.  Linearity: There must be a linear relationship between the dependent variable (regional GINI) and independent variable (GDP per capita).
    We can read from the two scatterplots below, FIG1 and FIG2, that the relationship for both Germany and Italy seems to be linear, with only some few outliers.
...
log(GDPpcn2): This variable measures the economic development of a region.
It is theoretically justified as higher GDP per capita regions tend to have lower levels of regional inequality, where from the multiple regression shows that if GDP per capita increase 1000 euro, leads to a higher level of inequality (gini_n2); 0.203\*\* for GE and 0.054 for IT.
