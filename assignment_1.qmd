...
```{r}
#| echo: FALSE
#| label: fig-GDPDENORTH
#| cap-location: bottom
#| fig-cap: "GDP plot North Germany"
merged_data |>
...
Tolkning Tyskland; **The zero values occur as a result of some regions are actually defined as a region, but in reality they are a foreign embassy, army base or offshore energy production with no permanent residence. This means they do not have any GDP per capita [@eurostat2021]. Moreover, we assume that the zero values can also be a result of missing data report from different regions in Germany.**
...
To calculate the population-weighted Gini coefficient for GDP for each European NUTS2 level region within Austria, Belgium, Bosnia and Herzegovina, Bulgaria, Croatia, Germany, Italy, Serbia, Sweden, we used the data on the GDP and population for the different regions and associated countries.
