...
*In this part we want to estimate a simple linear regression model for the year 2010, with regional development, measured in GDP, as the independent variable and regional inequality, measured in Gini, as the dependent variable. This is because we want to see the impact a change in the regional development will have on inequity in a given country.*
...
```{r}
lmDE <- lm(gini_n2 ~ I(GDP/100000), subset(merged_data, year == 2010 & nuts0 == "DE" & gini_n2 > 0))
lmIT <- lm(gini_n2 ~ I(GDP/100000), subset(merged_data, year == 2010 & nuts0 == "IT" & gini_n2 > 0))
...
ggplot(merged_data_filtered, aes(x = gini_n2, y = GDP/100000)) +
  geom_point() +
...
Velge hvordan vi ønsker å definere GDP.
